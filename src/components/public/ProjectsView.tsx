    ...
    <>
      <div>
        ...
        <p className="text-muted text-sm mb-6">
          Engineered systems, built with purpose.
        </p>
        ...
        {filtered.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-xl p-12 text-center">
            <p className="text-dim text-sm">
              {projects.length === 0
                ? "No projects yet. Add one in the admin panel."
                ...
                <div className="bg-card border border-border rounded-2xl overflow-hidden hover:border-green/30 hover:scale-[1.02] transition-all duration-200 group h-full">
                  {p.banner ? (
                    ...
                    <div className="w-full h-40 bg-gradient-to-br from-green/10 via-transparent to-transparent flex items-center justify-center">
                      <span className="font-mono text-3xl text-green/20">
                        {"{}"}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-bold text-text text-sm group-hover:text-green transition-colors">
                        {p.title}
                      </h3>

                      <span
                        className={`text-[10px] font-mono px-2 py-0.5 rounded-full shrink-0 ${STATUS_COLOR[p.status]}`}
                      >
                        {p.status}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {p.techStack.slice(0, 4).map((t) => (
                        <span
                          key={t}
                          className="font-mono text-[10px] bg-surface text-dim px-2 py-0.5 rounded"
                        >
                          {t}
                        </span>
                      ))}
      ...
      {recruiterMode && (
        <RecruiterMode onClose={() => setRecruiterMode(false)} />
      )}
    </>
  );
}
